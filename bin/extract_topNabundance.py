...
def extract_GenefamiliesAbundance(input_file, gstring):
    """
    Extract the community-level genefamilies abundance per sample.
    Store the unmapped reads per sample.
    """
    gf = input_file[~input_file[input_file.columns[0]].str.contains('\|')] # Extracting community-level genfamily/pathways abundance.
    ...
    gf = gf.loc[gf[gf.columns[0]] != 'UNMAPPED']
    return gf, Ureads
...
def topN_gfORpa_across_samples(gfORpa_samples, topN):
    """
    Estimate topN genefamilies or pathways 
    """
    gfORpa_samples.set_index(gfORpa_samples.columns[0], inplace = True) #Setting index at column name: ID.
    gfORpa_samples = gfORpa_samples.assign(total_abundance = lambda x: x.sum(axis = 1)) #Adding up the abudance of each gene family across the samples.
    gfORpa_samples_sort = gfORpa_samples.sort_values(by = ['total_abundance'], ascending = False) #Sort the samples based total abundance in decreasing order
    #gf_samples_sort.to_csv(f'{args.output}-sorted.csv')
    gfORpa_samples_sort = gfORpa_samples_sort.drop(columns = ['total_abundance']) #Dropping column total_abundance. Since its purpose is served. Not needed in future.
    
    # Select the top N rows from the sorted dataframe
    topNgfORpa_samples_abs = gfORpa_samples_sort.head(topN)
    
    #Estimation of others genefamily abundance within a sample and adding to the topNgf_samples_abs.
    topNgfORpa_samples_abs = topNgfORpa_samples_abs.T #transpose the topN genefamilies dataframe
    #Absoulute abundance of top N genefamilies including 'others' in each sample.
    topNgfORpa_samples_abs['Others'] = gfORpa_samples_sort.iloc[topN::].sum(axis = 0) # use the same sorted dataframe to estimate the abundance of others excluding tonp N genefamilies
    
    #Relative abundance of genefamilies within each sample.
    topNgfORpa_samples_rel = topNgfORpa_samples_abs.div(topNgfORpa_samples_abs.sum(axis = 1), axis = 0) #Estimating the relative abundance of each genefamily within a sample.

    return topNgfORpa_samples_abs, topNgfORpa_samples_rel
